...
def parse_verifier_response(text: str, checker: str) -> dict:
    ...
    raising, mirroring the defensive parsing in
    ``isolation.parse_diagnosis_response``/``parse_mutation_response``.
    ``aggregate_verdicts`` treats ``"error"`` as "this verifier's call
    itself failed", distinct from a genuine ``"pass"``/``"reject"``
    judgment.
    """
