...
class CreatorEvaluator(Evaluator):
    ...
    Uses llm run to spawn evaluation subagents for behavior testing.
    Falls back to LocalEvaluator if creator or LLM CLI unavailable.
    """
    ...
    def _claude_eval(self, skill_path: Path, gt_path: Path,
                     split: str) -> dict | None:
        """Use llm run to evaluate. Returns None if unavailable."""
        data = json.loads(gt_path.read_text())
        cases = data if isinstance(data, list) else data.get("evals", [])
        if split:
            cases = [c for c in cases if c.get("split", "dev") == split]
