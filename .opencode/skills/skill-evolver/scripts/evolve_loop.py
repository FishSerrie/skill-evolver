...
Phase 3 (Modify) use `llm run` subprocess to invoke LLM reasoning.
"""
...
LLM_BACKENDS = {
    "claude": {
        "cmd": ["llm", "run", "{prompt}", "--output-format", "text"],
        "model_flag": "--model",
...
def phase_2_3_ideate_and_modify(skill_path: Path, workspace: Path,
    ...
    """Phase 2+3: Use llm run to analyze failures and make an atomic change.
...
def run_l2_eval_via_claude(skill_path: Path, gt_path: Path,
    ...
    """Phase 5 L2: Use llm run to evaluate skill against GT cases.
...
def _local_eval(skill_path: Path, gt_path: Path) -> dict:
    """Fallback local eval when llm run is unavailable."""
    skill_content = (skill_path / "SKILL.md").read_text()
    gt_data = json.loads(gt_path.read_text())
    dev_cases = [c for c in gt_data.get("evals", []) if c.get("split", "dev") == "dev"]
...
def run_evolve_loop(skill_path: Path, gt_path: Path, workspace: Path,
    ...
    This is the REAL auto loop. Phase 2+3 use llm run for LLM reasoning.
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        # Phase 2+3: Ideate and Modify (via llm run)
        log("Phase 2+3: Ideate and Modify (calling llm run)")
